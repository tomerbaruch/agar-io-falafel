...
function balanceMass() {
    var totalMass = food.length * c.foodMass + users.map(function (u) {
        return u.mass;
...
function tickPlayer(currentPlayer) {
    ...
    var foodEaten = food.map(function (f) {
    ...
    var otherUsers = tree.retrieve(currentPlayer, function (user) {
        if (user.mass > 10 && user.id !== currentPlayer.id) {
            ...
            var collided = SAT.testCircleCircle(playerCircle, new C(new V(user.x, user.y), user.radius), response);
            if (collided) {
    ...
    playerCollisions.forEach(function (collision) {
        if (collision.aUser.mass > collision.bUser.mass * 1.1 && collision.aUser.radius > Math.sqrt(Math.pow(collision.aUser.x - collision.bUser.x, 2) + Math.pow(collision.aUser.y - collision.bUser.y, 2)) * 1.75) {
            ...
            if (util.findIndex(users, collision.bUser.id) > -1) users.splice(util.findIndex(users, collision.bUser.id), 1);
...
function gameloop() {
    if (users.length > 0) {
        ...
        for (i = 0; i < users.length; i++) {
            if (users[i].mass * (1 - c.massLossRate / 1000) > c.defaultPlayerMass) users[i].mass *= 1 - c.massLossRate / 1000;
        }
    }
    balanceMass();
}
...
function sendUpdates() {
    users.forEach(function (u) {
        var visibleFood = food.map(function (f) {
            if (f.x > u.x - u.screenWidth / 2 - 20 && f.x < u.x + u.screenWidth / 2 + 20 && f.y > u.y - u.screenHeight / 2 - 20 && f.y < u.y + u.screenHeight / 2 + 20) {
                return f;
            }
        }).filter(function (f) {
            return f;
        ...
        var visibleEnemies = users.map(function (f) {
            if (f.x > u.x - u.screenWidth / 2 - 20 && f.x < u.x + u.screenWidth / 2 + 20 && f.y > u.y - u.screenHeight / 2 - 20 && f.y < u.y + u.screenHeight / 2 + 20 && f.id !== u.id) {
                return {
                    id: f.id,
                    x: f.x,
                    y: f.y,
