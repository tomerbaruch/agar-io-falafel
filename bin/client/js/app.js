...
/***/ function(module, exports, __webpack_require__) {
	...
	function setupSocket(socket) {
	    ...
	    socket.on('leaderboard', function (data) {
	        ...
	        for (var i = 0; i < leaderboard.length; i++) {
	            status += '<br />';
	            if (leaderboard[i].id == player.id) status += '<span class="me">' + (i + 1) + '. ' + leaderboard[i].name + "</span>";else status += i + 1 + '. ' + leaderboard[i].name;
	        }
	...
	/* function drawCircle(centerX, centerY, radius, sides) {
		...
		ctx.drawImage(img,centerX,centerX,radius,radius);
	...
	function drawFalafel(centerX, centerY, radius, sides) {
	    var img = document.getElementById("scream");
	    var new_radius = radius * 6;
	    graph.drawImage(img, centerX - new_radius / 2, centerY - new_radius / 2, new_radius, new_radius);
	}
	...
	function drawFood(food) {
	    ...
	    drawFalafel(food.x - player.x + screenWidth / 2, food.y - player.y + screenHeight / 2, food.radius, 9);
	}

	function drawHumus() {
	    var img = document.getElementById("humus");
	    graph.drawImage(img, 0, 0, 5000, 5000);
	}
	...
	function drawPlayer() {
	    ...
	    var img = document.getElementById("pita");
	    x = screenWidth / 2 - (player.mass + 30) / 2;
	    y = screenHeight / 2 - (player.mass + 30) / 2;
	    graph.drawImage(img, x, y, player.mass + 30, player.mass + 30);
	    /*     var circle = {
	            x: screenWidth / 2,
	            y: screenHeight / 2
	        };
	...
	function gameLoop() {
	    if (died) {
	        ...
	        if (gameStart) {
	            ...
	            //drawHumus();
	            var img = document.getElementById("humus");
	            graph.drawImage(img, 0, 0, screenWidth, screenHeight);
